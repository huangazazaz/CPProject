        ...
        for filename in files:
            if filename.endswith(".spl"):
                ...
                # with open(output_file, "w") as out_file:
                    # subprocess.run(["bin/splc", input_file, '2'], stdout=out_file, stderr=out_file)
                with open(inter_file, "w") as out_file:
                    subprocess.run(["bin/splc", input_file], stdout=out_file, stderr=out_file)
    
    print("所有文件已处理，输出已保存到 'output' 文件夹中。\n")

# 传入input文件夹的路径，调用函数
input_folder = "test"  # 传入实际的input文件夹路径
